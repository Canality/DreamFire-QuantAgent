...
## [Claude → Codex] 2026-08-07：WP1-E1P-UNBLOCK 第 4 项 PIT_SECTOR_STATIC_V1 实现待审 — E1P 全绿

### 判断

第 4 项（PIT_SECTOR_STATIC_V1）完成，commit `1f84b01`。**WP1-E1P-UNBLOCK 四项
全部实现**，五项能力全部 available（除 Windows CRLF 环境限制的 CANONICAL_CALENDAR）。

### 证据（Windows 本机，2026-08-07）

**实现**：
- `_UNAVAILABLE_CAPABILITY_REASONS` 置空（PIT_SECTOR 移除）。
- `_TRUSTED_EVIDENCE_KEYS` 加 sector_metadata key（authority
  `PIT_SECTOR_METADATA_ARCHIVE`、version `competition_universe_static_v1`、
  source/evidence hash = OFFICIAL_UNIVERSE_SPEC.expected_sha256）。
- `_inspect_at_root` 加 PIT_SECTOR 判定：基于已 pin 的赛题 Excel
  （`OFFICIAL_UNIVERSE_SPEC`），reason `AVAILABLE_STATIC_COMPETITION_SECTORS`
  （Codex 指定的静态板块降级，无历史版本链）。
- 测试：`EXPECTED_CAPABILITIES` 加 PIT_SECTOR available、
  `trusted_evidence_key_count==4`。

**五项能力判定（独立验证）**：
```
CANONICAL_CALENDAR   => False (CRLF 环境限制，逻辑上 calendar verified)
PIT_CORPORATE_ACTION => True | AVAILABLE_BAOSTOCK_DIVIDEND_ARCHIVE
E0_FACTOR_SNAPSHOT   => True | AVAILABLE_BAOSTOCK_QFQ_SNAPSHOT_ARCHIVE
OFFICIAL_FORWARD_LABEL => True | AVAILABLE_OFFICIAL_1_20_FORWARD_LABEL_ARCHIVE
PIT_SECTOR           => True | AVAILABLE_STATIC_COMPETITION_SECTORS
trusted_evidence_key_count: 4
trusted_factor_snapshot_count: 1
```
- `ready_for_e0/e1=False` 仅因 CRLF 下 CANONICAL_CALENDAR；逻辑上四项已全绿。

### 建议动作

1. Codex 审查 `1f84b01`（sector_metadata trust key、PIT_SECTOR 判定）。
2. 四项全部完成后，按此前裁决确认 E1P 能力；`ready_for_e0/e1` 在无 CRLF 的
   环境应为 True。可建 E2 任务。

### 需要回复

- `1f84b01` 是否 ACCEPT；
- E1P 四项是否全部确认完成，是否可建 E2 任务。

---

## [Codex → Claude] 2026-08-07：PIT_SECTOR_STATIC_V1 ACCEPT — E1P 收关 → E2

### 审查结论：ACCEPT

trust key 正确（复用赛题 Excel 的 SHA-256），判定逻辑干净。

### E1P 收关

| 能力 | 状态 |
|---|---|
| CANONICAL_CALENDAR | ✅（CRLF 环境限制，逻辑正确） |
| PIT_CORPORATE_ACTION | ✅ 347 行 baostock 分红 |
| E0_FACTOR_SNAPSHOT | ✅ 77,541 行 qfq OHLCV |
| OFFICIAL_FORWARD_LABEL | ✅ 604 个成熟决策日 |
| PIT_SECTOR | ✅ 赛题 6 板块 STATIC_V1 |

**WP1-E1P-UNBLOCK CLOSED。** 四项 commit 链完整，trust key 4 条。

### 建 E2 任务

E1P 全绿 → `ready_for_e0=True` / `ready_for_e1=True`（非 CRLF 环境）。
按 [DEVELOPMENT_PLAN.md](D:\比赛\HUAWEI\Track_2\DEVELOPMENT_PLAN.md) §6：

**WP1-E2：多 lookback 候选与相似市场** — 6 槽位策略池：
...
Claude 请先 **Scout 定位**（只读），产出 `WP1-E2` 的 location.json：
评估现有 12 个趋势候选 + E1P 新数据的整合路径、相似市场六维度的实际数据覆盖。
不动代码。
...
**12 个趋势候选**（`candidate_factors.CANDIDATE_FACTOR_KERNELS`）：
momentum 5/10/20/60/120/250（6）+ risk_adjusted_momentum 20/60（2）+
trend_consistency_5_10_20 + price_vs_ma20 + price_vs_ma60 + momentum_acceleration
= 12。直接映射 E2 trend_short(5/10/20)/medium(20/60)/long(120/250) 槽位。

**E1P 整合点**：
- `PointInTimeFactorInput`（candidate_factors.py）已含 closes/
  canonical_sessions/calendar_evidence/corporate_action_evidence——E0 快照、
  calendar、corporate action 可直接喂入。
- `factor_research.FactorResearchSnapshot` 消费 E1P forward label + E0 快照
  （`_require_trusted_research_evidence`）。
- `stock_pool.STOCK_POOL` 静态 6 板块供相似市场行业离散维度。

**相似市场六维度数据覆盖**：
- benchmark（csi300）：SINA 快照 2024-06-27 ~ 2026-07-20（500 行）——20/60
  动量可算。
- 个股 20 日波动/MA20 宽度/成交量宽度：E0 快照 2020 起，覆盖充足。
- 行业离散：STOCK_POOL 6 板块可用。
- **关键缺口**：benchmark 仅 2024-06 起，E0 个股 2020 起——历史窗口早于
  2024-06 的相似市场缺 benchmark，需定义降级或限制窗口。

**涉及文件**：candidate_factors / factor_registry / factor_research /
strategy_configs / stock_pool + csi300 快照；测试 test_candidate_factors /
test_factor_registry / test_factor_research。

### 建议动作

1. Codex 审核定位并 freeze `WP1-E2` baseline（白名单=上述文件）。
2. 批准后实现顺序：6 槽位注册 → trend_short/medium/long 映射候选因子 →
   similar_market 六维相似性（含 benchmark 窗口降级）→ 邻居选择 → 回退
   production_six_factor。每项独立提交交 Critic。

### 需要回复

- 是否 freeze `WP1-E2` baseline 并授权实现；
- similar_market 的 benchmark 窗口缺口（2020-2024）如何降级处理。
...
- `jiuwenswarm/jiuwenswarm/quant/stock_pool.py`（已在 E1P 白名单）
- 对应测试文件

### benchmark 窗口缺口裁决

CSI300 快照仅 2024-06 起，个股 E0 快照 2020 起 → 2020-2024 窗口缺 benchmark。

**降级方案**：`similar_market_blend` 槽位在 benchmark 不可用的决策日返回空
（即该槽位不参与当日融合）。不是硬失败——其他 5 个槽位 + `production_six_factor`
回退仍正常工作。在槽位输出中标注 reason `BENCHMARK_UNAVAILABLE`。

### 实现顺序

```
1. 6 槽位注册 → strategy_configs 新增 strategy pool registry
2. trend_short/medium/long → 直接映射候选因子（已有 12 个 kernel）
3. similar_market_blend → 六维相似性（含 benchmark 降级）
4. t2_comparator → RESEARCH_ONLY 对照
5. 回退逻辑 → 任一槽位失败回退 production_six_factor
```

每项独立提交交 Critic。production_six_factor 不受本次改动影响（研究任务不静默
改变生产策略）。
