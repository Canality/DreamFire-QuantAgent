...
> 历史 79.7/81.7 分已确认受前视偏差污染，59-66 也只是自定义折算，均不是可信正式分。Phase A 已完成49只/6板块、500交易日、21个非重叠窗口的首日开盘固定股数统一评测；三个模型都只属于开发集证据。当前事实以根目录 `VALIDATION.md` 为准。

| 开发基线 | 中位20日收益 | 中位回撤 | 事前判定 |
|---|---:|---:|---|
| 生产六因子 | -0.07% | 3.42% | 当前生产对照 |
| 两因子 0.71/0.29 | +0.38% | 3.73% | 未通过最近4窗护栏 |
| momentum_20 only | +0.70% | 3.27% | 配对收益/胜率未通过 |
