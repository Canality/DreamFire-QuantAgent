...
当前生产代码仍保留历史 **6 因子模型**，但旧 81.7 分和旧 IC 已确认受前视偏差污染，不能再写“经验证”。
- **开发集候选**：momentum_20（aligned mean IC=+0.0787，Pos 72.7%）
- **开发集候选**：volume_trend（aligned mean IC=+0.0315，Pos 81.8%）
- 其余四因子当前判定为 REJECT；生产模型尚未切换，等待封存验证
- **风险约束**：波动率硬约束（vol_z > 2.0 → 排除）
- **权重方案**：市场状态（牛/熊/震荡）动态调整因子权重，量能因子市态不变
- **仓位分配**：风险平价方法，单只≤10%，单板块≤25%，最低现金≥5%

**已知局限性（必须在报告中披露）**：
- 因子筛选基于 11 个非重叠开发窗口；两个后续窗口仍封存，候选因子尚未完成验证
- momentum_20 在趋势市中表现最好，震荡/下跌市中预测力可能下降
- 20 日持仓周期上，基本面因子（PE/PB/ROE）IC≈0，不适用于本策略
- 已预留独立验证和最终留出窗口；在候选模型冻结前禁止打开最终留出
