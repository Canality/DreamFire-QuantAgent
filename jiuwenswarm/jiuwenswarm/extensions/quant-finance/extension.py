...
def _build_report_markdown(portfolio, backtest, regime, top_stocks):
    ...
    lines.extend([
        ...
        "- **因子模型**: 4 核心因子（经 IC 分析筛选）",
        "- **因子权重方案**: 根据市场状态动态调整",
        "",
        "| 因子 | 训练期 IC | 当前市态权重方向 | 说明 |",
        "|------|----------|----------------|------|",
        "| momentum_20 (20日动量) | +0.72 | 趋势市放大，震荡市压低 | 最强预测因子，趋势延续性最强 |",
        "| momentum_60 (60日动量) | +0.41 | 与 momentum_20 同向 | 中期趋势确认 |",
        "| reversal_5 (5日动量) | +0.39 | 震荡市提高权重 | 短期价格惯性 |",
        "| max_drawdown (最大回撤) | -0.38 | 所有市态均有效 | 回撤小的股票后续表现更好 |",
        "| volatility (波动率硬约束) | — | vol_z > 2.0 → 排除 | 降低尾部风险 |",
        "",
        "**选择理由**: 以上 4 因子是 IC 分析后保留的全部有效因子。PE/PB/ROE/RSI/volume_trend/turnover_momentum 等 7 个因子在 20 日窗口上 IC≈0，已移除。",
        "",
        '**假设声明**: 因子有效性基于训练期（2026 年 2-7 月）的 IC 测量。如果当前评测期市场状态与训练期显著不同，因子预测力可能下降。详见"模型局限性说明"章节。',
        "",
        "---",
        "",
        "## 三、投资组合明细",
